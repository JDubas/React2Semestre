...
export default function ModalAction(props) {
  document.title = props.idEditar > 0 ? "EDITAR" : "CADASTRAR";

  console.log("Id editar: " , props.idEditar);
  ...
  if (props.open) {
    return (
      <div className="container">
        <h1>{props.idEditar > 0 ? "EDITAR PRODUTO" : "CADASTRO DE PRODUTOS"}</h1>
        ...
        <span className="btnClose" onClick={()=> props.setOpen(false) }>X</span>
                    <legend>{props.idEditar > 0 ? "Editar produto" : "Cadastrar produto"}</legend>
                    <div>
